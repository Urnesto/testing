...
import { Pool, ProxyAgent, setGlobalDispatcher, Agent } from 'undici'
...
const XHR_HEADERS = {
  ...
  'accept-encoding': 'gzip, deflate',  // no brotli — undici auto-decompresses gzip/deflate but not br
...
// No brotli for detail pages — avoids decompression issues
...
let _proxyUrls  = []   // populated by initProxies() when --proxy is set
let _proxyAgents = []
let _proxyIdx   = 0
...
function initProxies() {
  try {
    ...
    logger.warn(`Proxy file not found: ${_PROXY_FILE}`)
    return
  }
...
function nextProxyAgent() {
  if (!_proxyAgents.length) return null
  return _proxyAgents[_proxyIdx++ % _proxyAgents.length]
}

// Make a request through a rotating proxy (falls back to direct if no proxies)
...
async function enrichJsonlWithImages() {
  ...
  logger.info(`Fetching images for ${parts.length} parts (concurrency=${IMG_CONCURRENCY})…`)

  // Dedicated pool — HTTP/2 multiplexes 50 streams over few connections (same as Python httpx)
  const imgPool = new Pool(SITE_ROOT, {
    connections: IMG_CONCURRENCY + 10,
    allowH2: true,
    headersTimeout: 30000,
    bodyTimeout: 60000,
    connectTimeout: 10000,
  })
  // Inject CF clearance cookie if available (acquired during scrape phase)
  const imgHeaders = _cfCookieHeader
    ? { ...DETAIL_HEADERS, cookie: _cfCookieHeader }
    : DETAIL_HEADERS

  const queue = new PQueue({ concurrency: IMG_CONCURRENCY })
  let done = 0
  let blocked = 0

  try {
    await Promise.all(parts.map(part => queue.add(async () => {
      if (_stop || !part.slug) { part.images = []; done++; return }

      const slug = part.slug.startsWith('http')
        ? part.slug
        : SITE_ROOT + (part.slug.startsWith('/') ? '' : '/') + part.slug
      ...
      try {
        ...
        if (!res) {
          const fetchRes = await fetch(slug, { headers: imgHeaders })
          part.images = fetchRes.ok ? extractImagesFromHtml(await fetchRes.text()) : []
        } else if (res.statusCode !== 200) {
          await res.body.dump()
          part.images = []
          blocked++
          if (blocked === 1 || blocked % 100 === 0) logger.warn(`Image requests blocked (non-200): ${blocked} so far (status=${res.statusCode})`)
        } else {
          part.images = extractImagesFromHtml(await readBodyText(res))
        }
      } catch (err) {
        logger.debug(`image ${String(slug).slice(-60)}: ${err.message}`)
        part.images = []
      }

      done++
      if (done % 500 === 0) logger.info(`Images: ${done}/${parts.length} (${(done / parts.length * 100).toFixed(0)}%) blocked=${blocked}`)
    })))
  } finally {
  ...
  const imgRate = parts.length / (imgElapsed || 1)
  const withImages = parts.filter(p => p.images?.length > 0).length
  logger.info(`Image enrichment done — ${parts.length} parts in ${imgElapsed.toFixed(1)}s (${imgRate.toFixed(1)} parts/s) | with_images=${withImages} blocked=${blocked} → ${OUTPUT_FILE}`)
}
...
async function runHttpSimple(pool, sem, probe, pagesTotal, concurrency) {
  ...
  for (let i = 0; i < remaining.length; i += batchSize) {
    ...
    const results = await Promise.allSettled(chunk.map(p => fetchPageHttp(pool, p, sem)))

    for (let j = 0; j < chunk.length; j++) {
      const pageNum = chunk[j]
      const r = results[j]
      if (r.status === 'rejected') {
        if (r.reason?.message === 'CF_CAPTCHA') {
  ...
  try {
    ...
    for (let catIdx = 0; catIdx < remainingCpcs.length; catIdx++) {
      ...
      for (let i = 0; i < remainingPages.length; i += batchSize) {
        ...
        const results = await Promise.allSettled(chunk.map(p => fetchPageHttp(pool, p, sem, cpc)))
        ...
        for (const r of results) {
          if (r.status === 'rejected') {
            ...
            // Failed page — count it as done (category mode skips second-pass)
          } else if (r.value) {
            for (const part of (r.value.parts || [])) writePart(part)
            pagesDone++
          }
...
async function runPlaywright(concurrency, cdpUrl, captchaWaitS = 0) {
  ...
  if (totalRows >= 10_000) {
    ...
    if (!_stop && existsSync(OUTPUT_FILE)) await enrichJsonlWithImages()
    return
  }
